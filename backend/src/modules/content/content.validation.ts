...
const contentBlockSchema = z.object({
  ...
  // Validación condicional: si el tipo es LIST, el valor debe ser un array de strings
  if (data.type === ContentBlockType.LIST) {
    return Array.isArray(data.value) && data.value.every((item: any) => typeof item === 'string');
  }
  // Si el tipo es FAQ, el valor debe ser un array de objetos con question y answer
  if (data.type === ContentBlockType.FAQ) {
    return Array.isArray(data.value) && data.value.every((item: any) =>
